...
/**
 * Valida se as credenciais do KV estão presentes.
 * @throws {Error} Se as credenciais estiverem faltando.
 */
function validateKvConfig() {
    if (!process.env.KV_REST_API_URL || !process.env.KV_REST_API_TOKEN) {
        throw new Error('CONFIGURAÇÃO AUSENTE: KV_REST_API_URL e/ou KV_REST_API_TOKEN não definidos. Verifique as variáveis de ambiente.');
    }
}
...
export async function saveTokens(tokens) {
    ...
    try {
        ...
        console.error('[saveTokens] Erro CRÍTICO ao salvar no KV:', error);
        throw new Error(`Falha ao salvar tokens no banco de dados (KV): ${error.message}`);
    }
}
...
export async function getTokens(memberId) {
    ...
    try {
        ...
        console.error('[getTokens] Erro ao ler do KV:', error);
        // Não lançamos erro aqui para não quebrar fluxos que podem tentar recuperar, mas logamos
        return null;
    }
}
